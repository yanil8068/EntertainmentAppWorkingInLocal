...
function MediaPlay({ singleMediaData }) {
  ...
  return (
    <div className="absolute h-full w-full top-0 left-0 flex items-center justify-center bg-black bg-opacity-60 rounded-lg">
      <button
        onClick={() =>
          navigate(`/${singleMediaData.mediaType}/${singleMediaData.id}`)
        }
        className="px-6 py-2 flex gap-3 items-center bg-gray-500 hover:bg-black text-lg font-semibold rounded-full duration-100"
      >
        <IoPlayCircle />
        <span>Play</span>
      </button>
