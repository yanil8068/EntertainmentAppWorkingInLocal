...
function MediaInfo({ singleMediaData }) {
  const mediaTypeCaptial = singleMediaData.mediaType
    ? singleMediaData.mediaType.charAt(0).toUpperCase() +
      singleMediaData.mediaType.slice(1)
    : "NA";
  return (
    <div className="px-1">
      <div className="flex gap-1 items-center">
        ...
        <span> {singleMediaData.mediaType ? mediaTypeCaptial : "NA"}</span>
        <LuDot />
        <span> {singleMediaData.isAdult ? "18+" : "PG"}</span>
      </div>
      <span className="font-bold">
        {" "}
        {singleMediaData.title ? singleMediaData.title : "NA"}
      </span>
    </div>
  );
}
