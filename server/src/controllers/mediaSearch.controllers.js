...
const findMovieController = async (req, res) => {
  try {
    ...
    // edit url & fetch data
    const updatedSearchMovieUrl = searchMovieUrl.replace(
      "query_Text",
...
// 02. search tv shows
const findTvController = async (req, res) => {
  try {
    // searchQuery from req parameters
    // searchQuery, variable name as in backend routing
    const { searchQuery } = req.params;
    ...
    // edit url & fetch data
    const updatedSearchTvUrl = searchTvUrl.replace(
      "query_Text",
      searchQuery.toString()
    );
    const data = await fetchData(
...
const findAllController = async (req, res) => {
  try {
    ...
    // edit url
    const updatedSearchAllUrl = searchAllUrl.replace(
      "query_Text",
      searchQuery.toString()
    );
