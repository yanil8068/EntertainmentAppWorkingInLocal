...
const trendingMedia = async (req, res) => {
  try {
    ...
    const data = await fetchData(
      trendingMediaUrl + page + `&api_key=${process.env.TMDB_KEY}`
    );
...
const movieMedia = async (req, res) => {
  try {
    ...
    const data = await fetchData(
      movieMediaUrl + page + `&api_key=${process.env.TMDB_KEY}`
    );
...
const tvMedia = async (req, res) => {
  try {
    ...
    const data = await fetchData(
      tvMediaUrl + page + `&api_key=${process.env.TMDB_KEY}`
    );
